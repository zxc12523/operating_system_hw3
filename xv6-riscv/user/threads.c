...
void my_thrdstop_handler(void *arg)
{
    ...
    // printf("thrd%d remain: %d context_id: %d\n", current_thread->ID, current_thread->remain_execution_time, current_thread->thrdstop_context_id);
    if (current_thread->remain_execution_time <= 0)
    {
        // printf("thrd%d remain time:%s\n", current_thread->ID, current_thread->remain_execution_time);
        thread_exit();
    }
    else
    {
...
void schedule(void)
{
    ...
    else if (current_thread->is_yield || current_thread->is_exited || tq == 3)
    {
        tq = 0;
        current_thread = current_thread->next;
    }
    else if (tq < 3 && current_thread->remain_execution_time > 0)
    {
        tq++;
        return;
    }
    ...
    else
    {
        ...
        struct thread *tmp1 = tmp;

        while (tmp != NULL)
        {
            if (tmp->remain_execution_time > 0 && tmp->start_time < start_time)
            {
                tmp1 = tmp;
    ...
    {
        int remain_time = 2147483647;
        struct thread *tmp = current_thread;
        struct thread *tmp1 = tmp;

        while (tmp != NULL)
        {
            if (tmp->remain_execution_time > 0 && tmp->remain_execution_time < remain_time)
            {
                tmp1 = tmp;
                remain_time = tmp->remain_execution_time;
            }
            tmp = tmp->next;
            if (tmp == current_thread)
                break;
